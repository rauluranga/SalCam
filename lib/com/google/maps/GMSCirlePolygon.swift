...
class GMSCirclePolygon: GMSPolygon {
    ...
    init(radius:Int, location:CLLocationCoordinate2D, detail:Int) {
        super.init()
        self.path = drawCirclePath(location, radius: radius, detail: detail)
    }

    convenience init (radius:Int, location:CLLocationCoordinate2D) {
        self.init(radius: radius, location: location, detail: 8)
    }
